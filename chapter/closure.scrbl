...
Safely calls the @tech{closure} @closure-lang-v9[value_c], invoking its @tech{code}, with
the arguments @closure-lang-v9[(value ...)].}
@item{@closure-lang-v9[(closure-ref value_c value_i)]

Deferences the value at index @closure-lang-v9[value_i] in the @tech{environment} of the
...
We first annotate all @closure-lang-v9[lambda]s with their free variable sets.
@racketblock[
`(lambda (,alocs ...) ,value)
=>
...
arguments, return procedures, or call an arbitrary variable with an arbitrary
number of arguments.
